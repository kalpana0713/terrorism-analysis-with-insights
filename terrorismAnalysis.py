# -*- coding: utf-8 -*-
"""
Created on Wed Sep  9 13:39:30 2020

@author: kalpana Patra
"""
  ...
  main_layout = html.Div(style={
              ...
              children=[
                  ...
                  dcc.Tabs(id="Tabs", value="Map",children=[
                      dcc.Tab(label="Map tool" ,id="Map tool",value="Map",style={
                          'bgcolor':'#4b5320'} ,children=[
          ...
          dcc.Dropdown(
              ...
              style={'border-radius': '100px', 'width':'60%','margin-left':'20%'},
              multi = True
        ),html.Br(),
         dcc.Dropdown(
              id='date', 
        ...
        html.Div([
         html.H3('SELECT THE YEAR', id='year_title', style={'text-align':'center', 'color':'red','background-color':'black'}),
         dcc.RangeSlider(
              id='year-slider',
              min=min(year_list),
              max=max(year_list),
         ...
         ],style = {'margin-top' : '80px','border-radius' : '10px', 'padding-top' : '10px' ,'backgroundColor': "black"}),
        html.Br(),
                  ]),
      ...
      dcc.Tab(label = "Chart Tool", id="chart tool", value="Chart", children=[
          dcc.Tabs(id = "subtabs2", value = "WorldChart",children = [
            ...
            dcc.Input(id="search", placeholder="Search Filter",
                      style={ 'border-radius':'100px','width':'20%'}),
            html.Hr(),
            html.Br(),
            html.Div([
            dcc.RangeSlider(
            ...
            ],style = {'margin-top' : '50px','border-radius' : '10px', 'padding-top' : '10px' ,'backgroundColor': "black"}),
                  html.Br(),
                 
                  
              ]),
         ]),
  dcc.Loading(id='graph',type='default')
  ])
...
def update_date(month):
    ...
    if month is  None:
        ...
        if any(elem in list1  for elem in month):
            
            b=( [{"label":m, "value":m} for m in date_list])
           
        elif any(elem in list2  for elem in month):
              b= ([{"label":m, "value":m} for m in date_list[:-1]])
              
        else: 
            b= ([{"label":m, "value":m} for m in date_list[:-2]])            
    return b  
...
#Main Function
def main():
  print("welcome to project insight...")
  load_data()
